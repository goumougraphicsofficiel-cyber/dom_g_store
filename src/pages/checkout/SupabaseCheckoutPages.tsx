...
import { OrderItemsCreationError, supabaseOrderService, type AdminOrder } from '../../services/supabaseOrderService'
...
function checkoutError(error: unknown) {
  if (error instanceof OrderItemsCreationError) return `${error.message} Le panier est conservé. Contactez l’assistance avec ce numéro.`
...
export function SupabaseCheckoutPage() {
  const { cart, products, user, authUser, updateUser, clearCart } = useStore()
    ...
    try {
      const result = await supabaseOrderService.create({
        ...
        items: lines.map(line => ({
          ...
          productSku: line.product.reference,
